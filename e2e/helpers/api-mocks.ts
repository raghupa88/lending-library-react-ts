...
export async function setupBooksApiMock(page: Page, response = MOCK_BOOKS) {
  await page.route(`${API_BASE}/books*`, (route) =>
    route.fulfill(fulfill(response))
  );
}

export async function setupLoginApiMock(page: Page, response = MOCK_LOGIN_SUCCESS) {
  await page.route(`${API_BASE}/auth/login`, (route) =>
    route.fulfill(fulfill(response))
  );
}

export async function setupRegisterApiMock(page: Page, response = MOCK_REGISTER_SUCCESS) {
